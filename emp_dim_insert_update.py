...
def join_and_detect_new_or_changed_rows():
    ...
    AWS_SECRET_KEY='AWS access'

    os.environ['AWS_ACCESS_KEY_ID']=AWS_ACCESS_KEY
    os.environ['AWS_SECRET_ACCESS_KEY']=AWS_SECRET_KEY

    emp_detail = pd.read_csv('s3://staging.emp.data/Abdelslam_emp_details.csv')
    emp_detail.drop(columns='Unnamed: 0', inplace=True)
    emp_sal = pd.read_csv('s3://staging.emp.data/Abdelslam_emp_sal.csv')
    ...
    sql_query = quaries.SELECT_DWH_EMP_DIM
